...
class Candidate:
    ...
    def addCandidate(self):
        ...
        for candidates in self._candidate_list:
            if candidates["candidate_name"]==cname and candidates["political_party"]==political_party and candidates["candidancy_from"]==c_from:
                candidate_registered=True
                break
        if candidate_registered:
            print(f"{'Candidate Already Registered':*^100}")
        else:
            self._candidate_list.append({"candidate_name":cname,"political_party":political_party,"candidancy_from":c_from})
            write_file(self._candidate_list,"candidatelist.json")
            print(f"Candidate:{cname} Political Party:{political_party} Candidancy from:{c_from} is registered")
    def showCandidate(self):
        self._candidate_list=read_file("candidatelist.json")
        if len(self._candidate_list)>0:
            print("#"*100)
            header = "{:^30}{:^30}{:^30}".format("Name of candidate".center(30), "Political Party".center(30), "candidacy from".center(30))
            print(header)
            for candidates in self._candidate_list:
                candidate_value = "{:^30}{:^30}{:^30}".format(candidates["candidate_name"].center(30), candidates["political_party"].center(30), candidates["candidancy_from"].center(30))
        ...
        if len(self._candidate_list)>0:
            ...
            for candidates in self._candidate_list:
                if candidates["candidate_name"]==cname and candidates["political_party"]==political_party and candidates["candidancy_from"]==c_from:
                    ...
                    break
            if candidate_registered:
                cname=input("Enter updated of candidate who participate in election:").lower()
                ...
                self._candidate_list.append({"candidate_name":cname,"political_party":political_party,"candidancy_from":c_from})
                write_file(self._candidate_list,"candidatelist.json")
                print(f"Candidate:{cname} Political Party:{political_party} Candidancy from:{c_from} is updated")
            else:
    ...
    def deleteCandidate(self):
        ...
        if len(self._candidate_list)>0:
            ...
            if candidate_registered:
                self._candidate_list.remove(candidates)
                write_file(self._candidate_list,"candidatelist.json")
                print(f"Candidate:{cname} Political Party:{political_party} Candidancy from:{c_from} is deleted")
            else:
                print(f"{'Candidate Not Registered':*^100}")
        else:
            print(f"{'Empty Candidate List':*^100}")
